...
def build_attack_df():
    path = "results/sglang/pin_chained_recovery_salted.csv"
    df = pd.read_csv(path)
    df['prompt'] = [
...
def run():
    ...
    pd.DataFrame(results, columns=['lcp_threshold', 'mutation_threshold', 'precision', 'recall', 'f1', 'fpr_slow', 'fpr_poweruser', 'fpr_shared_prefix']).to_csv(
        os.path.join(out_dir, "detection_threshold_sweep_salted.csv"), index=False)
    pd.DataFrame({'trial': range(len(lat)), 'requests_to_detect': lat}).to_csv(
        os.path.join(out_dir, "detection_latency_salted.csv"), index=False)

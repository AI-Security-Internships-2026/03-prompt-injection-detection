...
OUT_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "results", "sglang", "pin_position2_full.csv"))
...
def attacker_probe_position2(digit_guess: str):
    guess_digits = PLACEHOLDER_DIGIT + digit_guess + PLACEHOLDER_DIGIT * (PIN_LENGTH - 2)
...
def run():
    ...
    for trial in range(N_TRIALS):
        ...
        for digit in "0123456789":
            ...
            r = attacker_probe_position2(digit)
            r.update({
                "trial": trial,
                "probed_digit": digit,
                "ground_truth_pin": gt_pin,
                "ground_truth_digit2": gt_digit2,
            })
            rows.append(r)
            match = "MATCH" if digit == gt_digit2 else ""

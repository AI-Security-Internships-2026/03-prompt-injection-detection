...
# A fixed "template" the attacker knows the shape of, but not the exact secret content.
# In a real scenario the attacker wouldn't know this string - here we control it
# because we're the ones running both sides for verification purposes.
VICTIM_SECRET_PROMPT_TEMPLATE = "System context for user session: the confidential value is {secret}. Respond only with OK."
